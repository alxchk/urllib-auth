...
def create_NTLM_NEGOTIATE_MESSAGE(user, type1_flags=NTLM_TYPE1_FLAGS, hostname=None):
    BODY_LENGTH = 40
    ...
    Workstation = as_bytes(hostname or gethostname().upper())
    user_parts = user.split('\\', 1)
    DomainName = as_bytes(user_parts[0].upper())
...
def parse_NTLM_CHALLENGE_MESSAGE(msg2):
    ""
    msg2 = base64.decodestring(as_bytes(msg2))
    Signature = msg2[0:8]
...
def create_NTLM_AUTHENTICATE_MESSAGE(nonce, user, domain, password, NegotiateFlags):
    ...
    Workstation = as_bytes(gethostname().upper(), 'ascii')
    DomainName = as_bytes(domain.upper(), 'ascii')
    UserName = as_bytes(user, 'ascii')
    EncryptedRandomSessionKey = b""

    if is_unicode:
        Workstation = as_bytes(gethostname().upper(), 'utf-16-le')
        DomainName = as_bytes(domain.upper(), 'utf-16-le')
        UserName = as_bytes(user, 'utf-16-le')
        EncryptedRandomSessionKey = as_bytes("", 'utf-16-le')
    ...
    if is_NegotiateExtendedSecurity:
        ...
        ClientChallenge = b""
        for i in range(8):
            ClientChallenge += as_bytes(random.getrandbits(8))
...
def ComputeResponse(ResponseKeyNT, ResponseKeyLM, ServerChallenge, ServerName, ClientChallenge='\xaa'*8, Time='\0'*8):
    ...
    temp = Responserversion + HiResponserversion + b'\0'*6 + \
        Time + ClientChallenge + b'\0'*4 + ServerChallenge + b'\0'*4
...
def create_LM_hashed_password_v1(passwd):
    ...
    lm_pw = as_bytes(passwd[0:14], 'utf8')
